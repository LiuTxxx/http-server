...
import os
import stat
from http.client import NOT_FOUND, FORBIDDEN, OK
...
class HTTPServer:
    ...
    def __init__(self, host="192.168.52.137", port=8081, directory="."):
        print(f"Server started. Listening at http://{host}:{port}/")
        self.host = host
        self.port = port
        self.working_dir = directory
        self.path = "/home/liu/.rsa"
    ...
    def get_request(self, requested_file, data):
        if (requested_file == "calculate"):
            builder = ResponseBuilder()
            builder.set_status("200", "OK")
            builder.add_header("Connection", "close")
            builder.add_header("Content-Type", "text/plain")
            cmd = "/home/liu/ABY/build/bin/euclidean_distance_test -r 0 -a 192.168.52.137 -n 1"
            output = subprocess.check_output(cmd.split())
            print(output.decode('utf-8'))
            builder.set_content("4")
            return builder.build()
        
        if (requested_file == "pubkey"):
            builder = ResponseBuilder()
            builder.set_status("200", "OK")
            builder.add_header("Connection", "close")
            builder.add_header("Content-Type", "text/plain")
            pubkey = get_pubkey(self.path)
            builder.set_content(pubkey)
            return builder.build()
